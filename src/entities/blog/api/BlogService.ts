import { fetchAuth } from "../../../shared/api/ApiService"
import { IBlog, IBlogItem, IImage } from "../model/types"


class BlogService {

    async saveImage(blogId: string, imageData: ArrayBuffer): Promise<IImage> {
        const res = await fetchAuth(process.env.REACT_APP_SERVER_URL + `/blog/${blogId}/save_image`, {
            method: "POST",
            body: JSON.stringify({blogId, imageData}),
        })
        const {image}: {image: IImage} = await res.json()
        return image
    }

    async deleteImage(blogId: string, imageId: string){
        await fetchAuth(process.env.REACT_APP_SERVER_URL + `/blog/${blogId}/delete_image/${imageId}`, {
            method: "POST",
            body: JSON.stringify({blogId, imageId}),
        })
    }

    async getBlog(blogId: string): Promise<IBlog> {
        const res = await fetchAuth(process.env.REACT_APP_SERVER_URL + `/blog/${blogId}`)
        const blog: IBlog = await res.json()
        return blog
    }

    async updateBlog(blog: IBlog, body: string){
        await fetchAuth(process.env.REACT_APP_SERVER_URL + `/blog/${blog.blogId}/update`, {
            method: "POST",
            body: JSON.stringify({...blog, body}),
        })
    }

    async publishBlog(blogId: string, primaryImageId: string){
        await fetchAuth(process.env.REACT_APP_SERVER_URL + `/blog/${blogId}/publish`, {
            method: "POST",
            body: JSON.stringify({blogId, primaryImageId}),
        })
    }

    async unPblishBlog(blogId: string){
        await fetchAuth(process.env.REACT_APP_SERVER_URL + `/blog/${blogId}/publish`, {
            method: "POST"
        })
    }

    async createBlog(title: string){
        const res = await fetchAuth(process.env.REACT_APP_SERVER_URL + `/admin/blog/create`, {
            method: "POST",
            body: JSON.stringify({title})
        })
        const blogId: string = await res.json()
        return blogId
    }

    async getBlogs(): Promise<IBlogItem[]>{
        const res = await fetchAuth(process.env.REACT_APP_SERVER_URL + `/blog`)
        const {blogs}: {blogs: IBlogItem[]} = await res.json()
        return blogs
    }

}

export const blogService = new BlogService()