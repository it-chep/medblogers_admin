...
import { ICityItem } from '../../../../entities/doctor/model/types';
...
export const ChangeAdditionalDoctor: FC<IProps> = ({doctorId}) => {

    const [isLoading, setIsLoading] = useState<boolean>(true)
    const [cities, setCities] = useState<ICityItem[]>([])
    const [citiesDoctor, setCitiesDoctor] = useState<ICityItem[]>([])
    ...
    const onSelected = async (selected: ICityItem) => {
        const isInclude = citiesDoctor.findIndex(city => city === selected) >= 0
        if(isInclude) return
        await add(selected.id)
        const copy: ICityItem[] = JSON.parse(JSON.stringify(citiesDoctor))
        copy.push(selected)
