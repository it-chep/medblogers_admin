...
export const ChangeAdditionalSpeciality: FC<IProps> = ({doctorId}) => {
    ...
    const getData = async () => {
        try {
            ...
            if (e instanceof AuthError) {
                ...
                setGlobalMessage({message: 'Ошибка при получении данных о доп специальностях врача', type: 'error'})
            }
        } finally {
            setIsLoading(false)
        }
    ...
    const add = async (specialityId: number) => {
        try {
            ...
            await doctorService.addAdditionalSpeciality(doctorId, specialityId)
        } catch (e) {
            console.log(e)
            if (e instanceof AuthError) {
                setIsAuth(false)
        ...
        <section className={classes.container}>
            ...
            </section>
            {
                ...
                <section className={classes.list}>
                    {
                        specialitiesDoctor.map((spec, ind) => 
                            <section 
                                key={spec.id} 
