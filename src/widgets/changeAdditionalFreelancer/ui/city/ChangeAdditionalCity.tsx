...
import {freelancerService, ICityItemFreelancer} from '../../../../entities/freelancer';

interface IProps {
    freelancerId: number;
}

export const ChangeAdditionalCityFreelancer: FC<IProps> = ({freelancerId}) => {

    const [isLoading, setIsLoading] = useState<boolean>(true)
    const [cities, setCities] = useState<ICityItemFreelancer[]>([])
    const [citiesFreelancer, setCitiesFreelancer] = useState<ICityItemFreelancer[]>([])
    ...
    const getData = async () => {
        try {
            ...
            if (e instanceof AuthError) {
                ...
                setGlobalMessage({message: 'Ошибка при получении данных о доп города помощника', type: 'error'})
            }
        } finally {
            setIsLoading(false)
        }
    ...
    const add = async (cityId: number) => {
        try {
            ...
            await freelancerService.addAdditionalCity(freelancerId, cityId)
        } catch (e) {
            console.log(e)
            if (e instanceof AuthError) {
                setIsAuth(false)
    ...
    const onSelected = async (selected: ICityItemFreelancer) => {
        const isInclude = citiesFreelancer.findIndex(city => city === selected) >= 0
        if (isInclude) return
        try {
            await add(selected.id)
            const copy: ICityItemFreelancer[] = JSON.parse(JSON.stringify(citiesFreelancer))
            copy.push(selected)
            setCitiesFreelancer(copy)
        } catch (e) {
        }
    }

    const onDelete = async (ind: number) => {
        const targetCity = citiesFreelancer[ind];
        await freelancerService.deleteAdditionalCity(freelancerId, targetCity.id)
        const copy: ICityItemFreelancer[] = JSON.parse(JSON.stringify(citiesFreelancer))
        copy.splice(ind, 1)
        setCitiesFreelancer(copy)
    }
