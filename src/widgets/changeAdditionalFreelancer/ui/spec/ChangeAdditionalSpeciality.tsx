...
import { freelancerService, ISpecialityItemFreelancer } from '../../../../entities/freelancer';

interface IProps {
    freelancerId: number;
}

export const ChangeAdditionalSpecialityFreelancer: FC<IProps> = ({freelancerId}) => {

    const [isLoading, setIsLoading] = useState<boolean>(true)
    const [specialities, setSpecialities] = useState<ISpecialityItemFreelancer[]>([])
    const [specialitiesFreelancer, setSpecialitiesFreelancer] = useState<ISpecialityItemFreelancer[]>([])
    ...
    const getData = async () => {
        try {
            ...
            if (e instanceof AuthError) {
                ...
                setGlobalMessage({message: 'Ошибка при получении данных о доп специальностях помощника', type: 'error'})
            }
        } finally {
            setIsLoading(false)
        }
    ...
    const onSelected = async (selected: ISpecialityItemFreelancer) => {
        const isInclude = specialitiesFreelancer.findIndex(spec => spec === selected) >= 0
        if(isInclude) return
        try{
            await add(selected.id)
            const copy: ISpecialityItemFreelancer[] = JSON.parse(JSON.stringify(specialitiesFreelancer))
            copy.push(selected)
            setSpecialitiesFreelancer(copy)
        }
        catch(e){}
    }

    const onDelete = async (ind: number) => {
        const targetSpec = specialitiesFreelancer[ind];
        await freelancerService.deleteAdditionalSpeciality(freelancerId, targetSpec.id)
        const copy: ISpecialityItemFreelancer[] = JSON.parse(JSON.stringify(specialitiesFreelancer))
        copy.splice(ind, 1)
        setSpecialitiesFreelancer(copy)
    }
        ...
        <section className={classes.container}>
            ...
            </section>
            {
                ...
                <section className={classes.list}>
                    {
                        specialitiesFreelancer.map((spec, ind) => 
                            <section 
                                key={spec.id} 
                            ...
                            >
                                {spec.name}
                                <DeleteAction
                                    questionText='Вы точно хотите удалить доп специальность?'
                                    successText='Доп специальность успешно удалена'
                                    errorText='Ошбика при удалении доп специальности'
